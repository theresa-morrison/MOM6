...
module MOM_dynamics_split_RK2e
! e for embedded
...
subroutine step_MOM_dyn_split_RK2e(u_inst, v_inst, h, tv, visc, Time_local, dt, forces, &
                                  p_surf_begin, p_surf_end, uh, vh, uhtr, vhtr, eta_av, G, GV, US, CS, &
                                  calc_dtbt, VarMix, MEKE, thickness_diffuse_CSp, pbv, Waves, seaice)
  ...
  real, dimension(SZI_(G),SZJB_(G),SZK_(GV)) :: vold ! v-velocity before vert_visc is applied, for fpmix
                                                     !                                      [L T-1 ~> m s-1]
  real, dimension(SZIB_(G),SZJ_(G)) :: taux_surf_init 
  real, dimension(SZI_(G),SZJB_(G)) :: tauy_surf_init
  real, dimension(SZIB_(G),SZJ_(G)) :: taux_surf_pred 
  ...
  !--- OCEAN PREDICTOR STEP --- RK2A STYLE --- START ---!
...
! u_accel_bt = layer accelerations due to barotropic solver
  ...
  call btstep(u_inst, v_inst, eta, dt, u_bc_accel, v_bc_accel, forces, CS%pbce, CS%eta_PF, u_av, v_av, &
              ...
              taux_surf=taux_surf_init, tauy_surf=tauy_surf_init)
...
!   call do_group_pass(CS%pass_hp_uv, G%Domain, clock=id_clock_pass)
  ...
  !--- OCEAN PREDICTOR STEP --- RK2A STYLE --- END ---!
...
! Calculate the momentum forcing terms for the barotropic equations.
  ...
  call btstep(u_inst, v_inst, eta, dt, u_bc_accel, v_bc_accel, forces, CS%pbce, CS%eta_PF, u_av, v_av, &
              ...
              taux_surf=taux_surf_pred, tauy_surf=tauy_surf_pred)
  ...
  !--- OCEAN CORRECTOR STEP --- RK2A STYLE --- END ---!

  !--- OCEAN MISC --- RK2A STYLE --- START---!
...
! h_av = (h_in + h_out)/2 . Going in to this line, h_av = h_in.
  ...
  !--- OCEAN MISC --- RK2A STYLE --- END---!

  !--- OCEAN DIAGNOSTICS--- RK2A STYLE --- START ---!
  if (CS%fpmix) then
 ...
 !  call MOM_state_chksum("Corrector avg ", u_av, v_av, h_av, uh, vh, G, GV, US)
  ...
  !--- OCEAN DIAGNOSTICS--- RK2A STYLE --- END ---!
...
subroutine seaice_step(DS2d, G, sG, uo, vo, eta, dt_slow, taux, tauy)
  ...
  real, dimension(SZI_(G),SZJ_(G)),  intent(inout) :: eta      ! The free surface height
                                                               ! or column mass [H ~> m or kg m-2]
  ...
  !real, dimension(SZI_(G),SZJ_(G)),   :: sea_lev !< The height of the sea level, including
...
end module MOM_dynamics_split_RK2e
